...
public class PbEventDaoImpl extends BaseDaoImpl<PbEvent, Long>
	...
	public List<Map> findPbEventByDeviceId(String deviceId, Date startTime, Date endTime){
	...
	public StringBuffer baseSql() {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT DATE_FORMAT(u.record_time,'%Y-%m-%d') as dayTime,count(p.pb_id) as apneaSum,SUM(u.peroid)/3600 as peroidSum from pb_event p LEFT JOIN usetime u on(p.starttime=u.starttime)");
