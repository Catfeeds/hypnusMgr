...
public class LeakEventDaoImpl extends BaseDaoImpl<LeakEvent, Long>
	...
	public List<Map> findLeakEventByDeviceId(String deviceId, Date startTime, Date endTime){
	...
	public StringBuffer baseSql() {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT DATE_FORMAT(u.record_time,'%Y-%m-%d') as dayTime,count(p.leak_id) as apneaSum,SUM(u.peroid)/3600 as peroidSum from leak_event p LEFT JOIN usetime u on(p.starttime=u.starttime)");
