...
public class ApcpEventDaoImpl extends BaseDaoImpl<ApcpEvent, Long>
	...
	public List<Map> findApcpEventByDeviceId(String deviceId, Date startTime, Date endTime){
	...
	public StringBuffer baseSql() {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT DATE_FORMAT(u.record_time,'%Y-%m-%d') as dayTime,count(p.apapdd_id) as apneaSum,SUM(u.peroid)/3600 as peroidSum from apcp_event p LEFT JOIN usetime u on(p.starttime=u.starttime)");
