...
define('validation/1.13.1/validate',
    [ '$','validation/1.13.1/jquery.validate','validation/1.13.1/lib/jquery.metadata'],
    function(require, exports, module) {
        //需要考虑国际化
        var lang = (navigator.language || navigator.browserLanguage).toLowerCase();
        lang = lang.substring(0,lang.indexOf('-'));
        seajs.use('validation/1.13.1/localization/messages_' + lang);
        ...
        jQuery.validator.addMethod("isIdCardNo", function(value, element) {
            ...
            var isIdCardNo = function (num){
                ...
                if (len == 15){
                    ...
                    var D = new Date('19'+arrSplit[2]+'/'+arrSplit[3]+'/'+arrSplit[4]);
                    var B = D.getYear()==arrSplit[2]&&(D.getMonth()+1)==arrSplit[3]&&D.getDate()==arrSplit[4];
                    if (!B){
                        // 输入的身份证号里出生日期不对！
                        return false;
                    }
                    ...
                    var arrSplit = num.match(re);
                    var D = new Date(arrSplit[2]+"/"+arrSplit[3]+"/"+arrSplit[4]);
                    var B = D.getFullYear()==Number(arrSplit[2])&&(D.getMonth()+1)==Number(arrSplit[3])&&D.getDate()==Number(arrSplit[4]);
                    if (!B) {
