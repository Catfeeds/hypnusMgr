...
public class FlEventDaoImpl extends BaseDaoImpl<FlEvent, Long>
	...
	public List<Map> findFleEventByDeviceId(String deviceId, Date startTime, Date endTime){
	...
	public StringBuffer baseSql() {
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT DATE_FORMAT(u.record_time,'%Y-%m-%d') as dayTime,count(p.fl_id) as apneaSum,SUM(u.peroid)/3600 as peroidSum from fl_event p LEFT JOIN usetime u on(p.starttime=u.starttime)");
