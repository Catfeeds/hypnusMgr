...
var _69=Math.ceil(_68.total/_68.pageSize)||0;//||1; 默认为0页
var num=$(_67).find("input.pagination-num");
num.val(_68.total==0?0:_68.pageNumber);//num.val(_68.pageNumber);//没有记录，则页码也为0
num.parent().next().find("span").html(_68.afterPageText.replace(/{pages}/,_69));
var _6a=_68.displayMsg;
_6a=_6a.replace(/{from}/,_68.pageSize*(_68.pageNumber-1)+(_68.total==0?0:1));//+1);//没有记录，则从第0条开始
...
},beforePageText:"第",afterPageText:"页    共 {pages} 页",displayMsg:"当前显示第 {from} - {to} 条记录   共 {total} 条记录"};
})(jQuery);
(function($){
function _7a(_7b){
var _7c=$(_7b);
...
var t=$("<div title='"+((opts.tools[i].title)?opts.tools[i].title:"")+"'></div>").addClass(opts.tools[i].iconCls).appendTo(tool);//u by gt 加title
...
win.window("open");
},alert:function(_1f3,msg,icon,fn,opts){//opts对象参数
...
},confirm:function(_1f6,msg,fn,opts){//opts对象参数
...
},prompt:function(_1f9,msg,fn,opts){//opts对象参数
...
tab.unbind(".tabs").bind("click.tabs",{title:_25e},function(e){//tab被点击事件
if(!opts.isDrag){//a by gt 如果不可拖动
...
function  _initMenu(_2d0,menuObj){
	...
	if(_2d0){
		...
		if(!_2e2){//根据ID初始化根节点
			...
			if(menuObj.rootMenuWidth){//优先取参数的宽度
...
if(_2d8.parent){
	if(!_2d8.parent.submenu){
         ...
         //为新加的面板添加事件，所有面板在鼠标移进移出时操作同一个time变量
...
function cRow(_459,params){//a by gt 通过索引设置行勾选
...
function cRecord(_459,params){//a by gt 通过ID设置行勾选
	var opts=$.data(_459,"datagrid").options;
	var data=$.data(_459,"datagrid").data;
	if(opts.idField){
...
function rowIsChecked(_460,params){//a by zwb 通过索引获或者ID取行是否勾选
...
function checkChildren(nodeObj,id,boo,checks){
	...
	if(opts.checkChildren){
		var children=_5e3(nodeObj,id);//得到所有子节点
...
function checkParent(nodeObj,id,checks){
	...
	function getParents(nodeObj,id){//得到所有子节点
		var parent=_5e0(nodeObj,id);//单个子节点
		if(parent!=null){
			parents.push(parent);
...
//tr.addClass("datagrid-row-selected");//d by gt 树型列表全选
...
},onSelect:function(_56a,row){//a by gt  以下四个只是都声明，并无用处
...
if(!vv[0])vv.splice(0,1); //a by gt 去掉第1个空元素
$(_6ba).combo("setValues",vv);
if(!_6bc){
if(!ss[0]){ss.splice(0,1)} //a by gt 去掉第1个空元素
